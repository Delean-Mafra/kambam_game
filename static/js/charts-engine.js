/**
 * High-DPI Agile Metrics Canvas Charts Engine
 * Native HTML5 Canvas rendering for CFD, Lead Time, Finance, and Throughput.
 */
...
class ChartsEngine {
  ...
  renderCFD(canvasId, cfdHistory) {
    ...
    // Compute cumulative stacked points
    const stackedSeries = layers.map(() => []);

    cfdHistory.forEach(dayPoint => {
      let cumulative = 0;
      layers.forEach((layer, layerIdx) => {
        cumulative += (dayPoint[layer.key] || 0);
        stackedSeries[layerIdx].push(cumulative);
      });
    });

    // Draw stacked filled areas from top layer down to bottom
    for (let l = layers.length - 1; l >= 0; l--) {
      ctx.beginPath();
      ctx.fillStyle = layers[l].color + '44'; // Translucent fill
      ...
      // Bottom line (previous layer or bottom baseline)
    ...
    // Grid lines & labels
    this.drawAxes(ctx, padding, chartW, chartH, maxTotal, cfdHistory.map(d => d.day));
  }

  renderLeadTime(canvasId, deliveredCards) {
    const canvas = document.getElementById(canvasId);
    const setup = this.setupCanvas(canvas);
    if (!setup) return;
    const { ctx, width, height } = setup;
    ...
    // Bars/Points per delivered card
    ...
    deliveredCards.forEach((c, idx) => {
      ...
      // Lead time value on top
      ctx.fillStyle = '#cbd5e1';
      ctx.font = '10px monospace';
      ctx.textAlign = 'center';
      ctx.fillText(`${lt}d`, x, y - 4);

      // Card code label bottom
      ctx.fillStyle = '#64748b';
      ctx.font = '9px monospace';
      ctx.fillText(c.code ? c.code.replace('CRD-', '#') : `#${idx + 1}`, x, height - 8);
    });

    this.drawAxes(ctx, padding, chartW, chartH, maxLead, null, 'd');
  }

  renderFinancial(canvasId, history) {
    const canvas = document.getElementById(canvasId);
    const setup = this.setupCanvas(canvas);
    if (!setup) return;
    const { ctx, width, height } = setup;
  ...
  renderThroughput(canvasId, dailyDelivered) {
    const canvas = document.getElementById(canvasId);
    const setup = this.setupCanvas(canvas);
    if (!setup) return;
    const { ctx, width, height } = setup;
  ...
  drawAxes(ctx, padding, chartW, chartH, maxY, xLabels = null, unit = '') {
